/*
月饼是中国人在中秋佳节时吃的一种传统食品，不同地区有许多不同风味的月饼。现给定所有种类月饼的库存量、总售价、以及市场的最大需求量，请你计算可以
获得的最大收益是多少。
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        cin >> m[i].sale;
        m[i].p = m[i].sale / m[i].k;
    }
    sort(m + 1, m + 1 + n, cmp); //按单价从高到低排序
    for(int i = 1; i <= n && d >= m[i].k; i++) {
        maxp += m[i].sale;
        d -= m[i].k;
        t = i;
    }
    if(t != n) maxp += m[t + 1].p * d; //剩余需求量
    printf("%.2f", maxp);
    return 0;
}